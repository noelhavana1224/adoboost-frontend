...
function TestEmailModal({ open, onClose, sequences, accounts = [], rotationIds = [] }) {
  ...
  const handleSend = async () => {
    ...
    if (!selectedAccountId) return toast.error('No email account selected on this campaign');
    if (!seq.subject) return toast.error('This step has no subject line');
      ...
      <div style={{ padding:'4px 0' }}>
        ...
        {emailSeqs.length > 1 && (
          <div style={{ marginBottom:16 }}>
            <label style={{ fontSize:13, fontWeight:600, color:'var(--text2)', display:'block', marginBottom:6 }}>Step to test</label>
            <select value={stepIdx} onChange={e => setStepIdx(Number(e.target.value))}
        ...
        {fromAccount && (
          <div style={{ marginBottom:16, padding:'10px 13px', background:'var(--bg3)', borderRadius:8, fontSize:12, color:'var(--text3)' }}>
            Sending from: <b style={{ color:'var(--text)' }}>{fromAccount.from_name || fromAccount.name}</b> &lt;{fromAccount.from_email || fromAccount.username}&gt;
          </div>
        )}
        <div style={{ marginBottom:8 }}>
          <label style={{ fontSize:13, fontWeight:600, color:'var(--text2)', display:'block', marginBottom:6 }}>Send test to</label>
          <input type="email" placeholder="your@email.com" value={toEmail}
            onChange={e => setToEmail(e.target.value)}
            onKeyDown={e => e.key === 'Enter' && handleSend()}
            style={{ width:'100%', padding:'10px 13px', border:'1px solid var(--border2)', borderRadius:8, fontSize:14, fontFamily:'inherit', outline:'none', color:'var(--text)' }}
            autoFocus />
        </div>
