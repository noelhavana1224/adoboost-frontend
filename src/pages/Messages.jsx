...
import { MessageSquare, Search, Tag, Filter, Reply, ChevronDown, ChevronUp, X } from 'lucide-react';
...
export default function Messages({ type = 'inbox' }) {
  ...
  return (
    <div>
      <PageHeader
        title="Messages"
        subtitle={type === 'inbox' ? 'Track and reply to campaign responses' : 'Auto-reply responses filtered automatically'}
      />

      {/* Tabs */}
      <div style={{ display: 'flex', gap: 0, borderBottom: '2px solid var(--border)', marginBottom: 20 }}>
        {[{ label: '📥 Inbox', path: '/messages/inbox' }, { label: '⚙️ Auto-replies', path: '/messages/auto-replies' }].map(t => (
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 16, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <button onClick={() => setFilterTag('')} style={{
            padding: '5px 12px', borderRadius: 20, border: `1px solid ${!filterTag ? 'var(--primary)' : 'var(--border2)'}`,
      ...
      {loading ? <Spinner /> : messages.length === 0 ? (
        <Empty icon={MessageSquare}
          ...
          description={filterTag ? 'Try a different filter or clear the tag filter.' : 'Replies from your campaigns will appear here.'} />
      ) : (
          ...
          {messages.map((m, i) => {
            ...
            return (
              <div key={m.id} style={{
                ...
                background: m.tag === 'positive' ? '#f0fff4' : m.tag === 'meeting_booked' ? '#f5f3ff' : 'transparent',
                ...
                <div style={{ padding: '12px 16px', display: 'grid', gridTemplateColumns: '1fr auto', gap: 12, alignItems: 'start' }}>
                  <div style={{ minWidth: 0 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4, flexWrap: 'wrap' }}>
                      <span style={{ fontWeight: 600, fontSize: 14 }}>{m.from_name || m.from_email}</span>
                    ...
                    {/* Subject */}
                    <div style={{ fontSize: 13, fontWeight: 500, marginBottom: 2, color: 'var(--text)' }}>{m.subject || '(no subject)'}</div>

                    {/* Meta */}
                    <div style={{ fontSize: 12, color: 'var(--text3)', display: 'flex', gap: 12 }}>
                      <span>📢 {m.campaign_name || 'Unknown campaign'}</span>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6, alignItems: 'flex-end', flexShrink: 0 }}>
                    ...
                    {type === 'inbox' && !detectedAutoReply && (
                      <button onClick={() => setReplyModal(m)} style={{
